...
class DBSentinelUtility:
    ...
    def _run_parallel_comparison(self, tables: List[Dict]) -> Dict:
        ...
        # Pre-validate all tables before parallel processing
        self.logger.info("Pre-validating table access...")
        for table in tables:
            ...
            try:
                ...
                if not self.db_manager.test_table_exists(source_conn, source_schema, table_name):
                    raise Exception(f"Source table {source_schema}.{table_name} not accessible")
                
                # Test target table access  
                target_conn = self.db_manager.get_target_connection()
                if not self.db_manager.test_table_exists(target_conn, target_schema, table_name):
                    raise Exception(f"Target table {target_schema}.{table_name} not accessible")
                
                self.logger.info(f"✅ Table validation passed: {table_name}")
                
            except Exception as e:
                self.logger.error(f"❌ Table validation failed: {table_name} - {str(e)}")
                comparison_results[table_name] = {
                    'status': 'ERROR',
                    'table_name': table_name,
                    'error': f"Table validation failed: {str(e)}",
                    'differences': {'inserts': [], 'updates': [], 'deletes': []}
                }
        
        # Only process tables that passed validation
        valid_tables = [table for table in tables if table['table_name'] not in comparison_results]
        
        if not valid_tables:
            self.logger.error("No valid tables found for comparison")
            progress_bar.close()
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for table in valid_tables:
                try:
                    future = executor.submit(self._compare_table_with_error_handling, table, progress_bar)
                    future_to_table[future] = table
                    self.logger.debug(f"Submitted comparison task for table: {table['table_name']}")
                except Exception as e:
                    self.logger.error(f"Failed to submit task for table {table['table_name']}: {str(e)}")
                    comparison_results[table['table_name']] = {
                        'status': 'ERROR',
                        'table_name': table['table_name'],
            ...
            for future in as_completed(future_to_table):
                ...
                try:
                    ...
                    if result.get('status') == 'SUCCESS':
                        ...
                        self.logger.error(f"❌ Comparison failed for table: {table_name} - {result.get('error', 'Unknown error')}")
                    
                except Exception as e:
                    self.logger.error(f"❌ Error in comparison task for table {table_name}: {str(e)}")
                    comparison_results[table_name] = {
                        'status': 'ERROR',
                        'table_name': table_name,
                        'error': f"Comparison task error: {str(e)}",
                        'differences': {'inserts': [], 'updates': [], 'deletes': []}
                    }
    ...
    def _compare_table_with_error_handling(self, table_config: Dict, progress_bar: tqdm) -> Dict:
        """
        Compare a single table with comprehensive error handling.
        ...
        try:
            self.logger.debug(f"Starting comparison for table: {table_name} in thread {threading.current_thread().ident}")
            
            # Create a new database manager instance for this thread to avoid connection conflicts
            thread_db_manager = DatabaseManager(self.config)
            
            # Test connections in this thread
            source_conn = thread_db_manager.get_source_connection()
            target_conn = thread_db_manager.get_target_connection()
            ...
            try:
                ...
                self.logger.debug(f"Thread {threading.current_thread().ident} - Source user: {source_user}, Target user: {target_user}")
            except Exception as e:
                self.logger.warning(f"Could not determine database users: {str(e)}")
            
            # Create comparator with thread-specific database manager
            comparator = TableComparator(self.config, thread_db_manager)
            result = comparator.compare_table(table_config, progress_bar, self.progress_lock)
            
            # Clean up thread-specific connections
            thread_db_manager.close_thread_connections()
            
            return result
            
        except Exception as e:
            self.logger.error(f"Error in table comparison for {table_name}: {str(e)}", exc_info=True)
            return {
                'status': 'ERROR',
                'table_name': table_name,
                'error': str(e),
